...
<div class="main__inner">
    ...
    <form action="{{ route('admin.attendance.approve', $attendance->id) }}" method="POST">
        ...
        <table class="attendance-detail__table">
                ...
                <td class="attendance-detail__table-row--value">
                    <div class="time-group">
                        <span class="time-group--item">
                            <p class="time-group--item-inner">{{ date('H:i', strtotime($attendance->punch_in)) }}</p>
                        </span>
                        <span class="time-separator">〜</span>
                        <span class="time-group--item">
                            <p class="time-group--item-inner">{{ $attendance->punch_out ? date('H:i', strtotime($attendance->punch_out)) : '' }}</p>
                        </span>
                    </div>
                </td>
            </tr>
            ...
            <tr class="attendance-detail__table-row">
                <th class="attendance-detail__table-row--title">休憩{{ $index > 0 ? $index + 1 : '' }}</th>
                <td class="attendance-detail__table-row--value">
                    <div class="time-group">
                        <span class="time-group--item">
                            <p class="time-group--item-inner">{{ date('H:i', strtotime($rest->start_time)) }}</p>
                            <p class="time-separator">〜</p>
                            <p class="time-group--item-inner">{{ $rest->end_time ? date('H:i', strtotime($rest->end_time)) : '' }}</p>
                        </span>
                    </div>
                </td>
            </tr>
            ...
            <tr class="attendance-detail__table-row">
                ...
                <td class="attendance-detail__table-row--value">
                    <p class="attendance-detail__table-row--value-remarks">{{ $attendance->remarks }}</p>
                </td>
            </tr>
        </table>
